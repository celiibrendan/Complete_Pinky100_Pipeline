...
#(schema.jobs & "table_name='__component_label_orphan'").delete()
...
# spine_orphan_errors = pinky.ComponentLabelOrphan() & "n_heads = 0" & "n_neck=0" & "n_spine=0" & "n_stub=0" & "n_error=0"


# In[ ]:


# spine_orphan_errors.delete()
...
class ClassifyMesh(object):
    ...
    #import the mesh
        ...
        print("after import from trimesh in ")
        print("len(self.mesh.faces) = " + str(len(self.mesh.faces)))
        print("len(self.mesh.vertices) = " + str(len(self.mesh.vertices)))
...
def fix_verts_labels(filtered_verts,filtered_faces,final_faces_labels_list):
    """
    Function that maps the final faces to the vertices because
    the trimesh import affects the number of vertices in the mesh
    and returns a different numbered vertices
    ...
    #print("Counter(final_spine_labels) = " + str(Counter(final_faces_labels_list)))
    #assign the labels to the correct faces
    for i,fi in enumerate(final_faces_labels_list):
        #assign the vertices an index
        for vert in filtered_faces[i]:
            final_verts_labels_list[vert] = fi
    
    
    return final_verts_labels_list
...
#pinky.ComponentLabelOrphan.drop()
...
#define the table
@schema
class ComponentLabelOrphan(dj.Computed):
    definition = """
    # creates the labels for the mesh table with the head,neck,spine,error,stub stats
   ...
   """
    ...
    def make(self, key):        
        ...
        verts,faces = (pinky.Decimation35OrphanStitched & original_mesh_key).fetch1("vertices",
                                                                                                 "triangles")
        ...
        component_vertices, component_faces = (pinky.CompartmentOrphan.ComponentOrphan & component_mesh_key).fetch1("vertex_indices",
                                                                                                      "triangle_indices")
...
#         print("filtered_verts = " + str(filtered_verts))
#         print("filtered_faces = " + str(filtered_faces))

        """
        Function that maps the final faces to the vertices because
        the trimesh import affects the number of vertices in the mesh
        and returns a different numbered vertices, so this maps the labels manually
        to the vertices that were pulled from Compartment.Component

        """
        
        final_verts_labels_list = fix_verts_labels(filtered_verts,filtered_faces,final_faces_labels_list)
        
        #raise error if not the same
        if len(component_vertices) != len(final_verts_labels_list): 
            print(f"len(component_vertices) = {len(component_vertices)} + len(final_verts_labels_list) = {len(final_verts_labels_list)}")
            raise Exception("spine vertices len don't match length of component vertices")
            
        if len(component_faces) != len(final_faces_labels_list):
            print(f"len(component_faces) = {len(component_faces)} + len(final_faces_labels_list) = {len(final_faces_labels_list)}")
            raise Exception("spine faces don't match length of component faces")
...
start_time = time.time()
ComponentLabelOrphan.populate(reserve_jobs=True)
