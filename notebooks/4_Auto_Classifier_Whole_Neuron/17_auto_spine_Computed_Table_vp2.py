...
def load_Neuron_automatic_spine(ID,compartment_type,compartment_index):
    ...
    neuron_data = ((ta3.CleansedMesh & primary_key & "segment_ID="+str(ID)).fetch(as_dict=True))[0]
...
def get_cgal_data_and_label(neuron_ID, decimation_ratio, compartment_type ,component_index,clusters,smoothness,ob_name):
       
    #store the group_segmentation in the traingle labels from datajoint
    
    
    #clusters=clusters,smoothness=smoothness
    comp_dict = dict(segmentation=1,
    ...
    component_data = (ta3.ComponentAutoSegment40() & [comp_dict]).fetch(as_dict=True)
    if component_data == []:
        return [], []
    else:
    ...
    for key,value in sdf_temp_dict.items():
        ...
        sdf_final_dict[key] = np.median(value)
...
def _make_tuples(key):
    ...
    for clusters in cluster_list:
        ...
        ob_name = load_Neuron_automatic_spine(neuron_ID,compartment_type,component_index)
        ...
        sdf_final_dict, labels_list = get_cgal_data_and_label(neuron_ID,decimation_ratio, compartment_type ,component_index,clusters,smoothness,ob_name)
