...
class CompartmentFinal(dj.Computed):
    ...
    key_source = ta3p100.CleansedMesh & ta3p100.CurrentSegmentation & 'decimation_ratio=0.35' & ta3p100.CoarseLabelFinal.proj()
    ...
    def make(self, key):
        ...
        labels = (ta3p100.CoarseLabelFinal & key).fetch1()
        #print("something")
        if len(np.unique(labels['triangles'])) == 1:
            #print("heyo")
            self.insert1(key)
            label_name = (ta3.LabelKey & dict(numeric=np.unique(labels['triangles'])[0])).fetch1('description')
